...
/**
 * Describes a source and a filters used to arrive to a set of submissions to
 * analyze.
 * 
 * @author mfreire
 */
public class SourceSet implements XMLSerializable {
	...
	/**
	 * The ugly way to initialize a sourceSet. Discards all filter information, and
	 * exports everything to a temporary folder.
	 * 
	 * @param dn
	 */
	...
	public Element saveToXML() throws IOException {
		...
		for (FileTreeNode sourceRoot : sourceRoots) {
			...
			rootElement.setAttribute("sha1", Hasher.showBytes(h.hash(sourceRoot.getFile()).getSha1()));
	...
	private void loadFilter(Element filterElement, CompositeFilter target) throws IOException {
		target.clear();
	...
	/**
	 * @return the filtered tree. If you suspect that it may not be up-to-date, run
	 *         buildFilteredTree first.
	 */
	...
	/**
	 * Writes (exports) a FileTreeNode's contents to a directory. This allows all
	 * filters and roots to be discarded on the result.
	 * 
	 * @param sourceTree      to export
	 * @param targetDirectory where it will be saved
	 * @throws IOException
	 */
	public void export(FileTreeNode sourceTree, File targetDirectory) throws IOException {
...
//				log.info("child for root " + dn.getLabel() + ": " + fn.getLabel());
	...
	/**
	 * Load mount points given the roots element in sources node. After this call,
	 * every file under each mount point will be added to the sources field of this
	 * instance.
	 *
	 * @param rootsElement roots element in sources node
	 * @throws java.io.IOException if mount points can't be loaded, for example when
	 *                             a wrong digest is found.
	 */
		...
		for (Element e : roots) {
			try {
				...
				if (!sha1.equals(digest)) {
					...
					throw new IOException("Wrong checksum for " + path + ". Found something starting with "
							+ digest.substring(0, 10) + ", but expected " + sha1);

				} else if (!path.exists()) {
					throw new IOException("Mount point '" + path.getAbsolutePath() + "' not found.");
				}

				if (!path.isDirectory()) {
					System.err.println("Found mount point '" + path.getAbsolutePath() + "' but it isn't a  directory.");
				}
